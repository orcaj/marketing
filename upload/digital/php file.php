<?php
    ...
    class Model_Payment extends Model {
        ...
        static function find($case_id, $id){
            // $result = \DB::select()->from('payments')
            //     ->where('case_id', '=', $case_id)
            //     ->where('active', '!=', 0)
            //     ->where('id', '=', $id)->execute();
            // return current($result->as_array());

            $result=Payment::where(['case_id' => $case_id,'id'=>$id])->where('active', '!=', 0)->get();
            return $result;
        }
        ...
        static function getTotalPayments($case_id, $all_scheduled = false){
            ...
            $query = \DB::select('id', 'amount')->from('payments')->where('case_id', '=', $case_id)->where('active', '!=', 0);
